...
public class MainClass {
    public static void  main(String[] args )
    {
        ...
        Actions.clickon(select.waitForElementToBeClickable(By.className("f16sug5q")));//click on bar
        Actions.SendInput_Enter(select.findByCssSelector("#bigsearch-query-location-input"),location);//send input text
        ...
        Actions.clickon(select.findByClassName("pz9siyu"));//click on gesut icon
        Actions.clickon(select.data_testid("stepper-adults-increase-button"),2);//click with number
        ...
        if(select.TextContainsSubstring(select.waitForElementToBePresent(By.xpath("/html/body/div[5]/div/div/div[1]/div/div[2]/div[2]/main/div[2]/div/div[1]/div/div/div/section/h1/span")),city))
        {System.out.println("Result of search header contains " + city);headerresult=true;}
        else {System.out.println("Error! the header result dose not search criteria");}

        boolean fltrplace=false;
        if(select.TextContainsSubstring(select.findByClassName("f16sug5q"),city))
        {System.out.println("Filter contains Rome "+city);fltrplace=true;}
        ...
        {System.out.println("Filter contains correct check out day " + checkinday);dateout=true;}
        else{System.out.println("Error! the check in day out filter area dose not search criteria");}

        boolean gesutnumb=false;
        ...
        {System.out.println("Filter contains  " + "3");gesutnumb=true;}
        else{System.out.println("Error! the number of guestes  in filter area dose not search criteria");}
        ...
        Actions.clickon(select.waitForElementToBeClickable(By.className("l1d3yagp")));//wait for showmore
        ...
        List<WebElement> innerDivs =select.findAllByLocator(By.className("c4mnd7m"));//list of all elements
        ...
        for (WebElement element : innerDivs)
        {
            try
            {
                ...
                String spanText = (element.findElement(By.cssSelector("div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(4) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(2) > div:nth-child(3) > span:nth-child(2) > span:nth-child(3)"))).getText();//text contain number of bedrooms
                ...
                if ((bedrooms))//if text contains bedroom and no wrong word
                {
                    ...
                    else {System.out.println("Error! property number: "+generalcounter+" doesn't have least number of bedrooms."+"bedwordis ");}//less than 5
                }
                if (bedrooms && leastnumber)
                {containsbedroomcounter++;}//inc number of correct items
        ...
        Actions.clickon(selectnew.waitForElementToBeVisiblebyxpath("/html/body/div[10]/section/div/div/div[2]/div/div[1]/button"));//close popup
        Actions.clickon(selectnew.findByCssSelector("#site-content > div > div:nth-child(1) > div:nth-child(3) > div > div._16e70jgn > div > div:nth-child(6) > div > div:nth-child(2) > section > div.b9672i7.dir.dir-ltr > button"));//open popup


        List<String> listaaa=selectnew.findListOfStrings("twad414");//list of string in popup
        for(String ss:listaaa)
        {
            if (ss.contains("Pool")||ss.contains("Private pool")||ss.contains("Private outdoor pool"))
            {
